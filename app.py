...
def start_scheduler():
    ...
    job = scheduler.add_job(func=generate_maintenance_tasks, trigger="interval", days=1, next_run_time=datetime.now())
    scheduler.start()
...
def generate_maintenance_tasks():
    ...
    if existing_tasks:
        print("As tarefas de manutenção já foram geradas para hoje. A função não será executada novamente.")
    ...
    for machine in machines:
        ...
        if series in machine_series_manuals:
            ...
            send_checklist_to_motorista(motorista_name, motorista_email, maintenance_tasks, motorista_id)
...
def list_machines():
    ...
    return render_template('machines/list.html', machines=machines, gerentes_por_maquina=gerentes_por_maquina)
...
def add_machine():
    ...
    if request.method == 'POST':
        ...
        with connect_db() as conn:
            cursor = conn.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            ''', (user_id, model, serial_number, purchase_date, other_details, motorista_id))
    ...
    return render_template('machines/add.html', auxiliaries=auxiliaries, machine_models=MACHINE_MODELS)
...
def edit_machine(machine_id):
    ...
    with connect_db() as conn:
        ...
        machine = conn.execute('SELECT * FROM machines WHERE id = ? AND user_id = ?', (machine_id, user_id)).fetchone()
        if not machine:
            flash('Máquina não encontrada.')
            return redirect(url_for('list_machines'))

        # Obter IDs dos gerentes associados à máquina
        gerente_ids = [row['gerente_id'] for row in conn.execute('''
            SELECT gerente_id FROM machine_managers WHERE machine_id = ?
        ''', (machine_id,)).fetchall()]
        ...
        if request.method == 'POST':
            ...
            ''', (model, serial_number, purchase_date, other_details, motorista_id, machine_id, user_id))

            # Atualizar os gerentes associados
            conn.execute('DELETE FROM machine_managers WHERE machine_id = ?', (machine_id,))
    ...
    return render_template('machines/edit.html', machine=machine, auxiliaries=auxiliaries, gerente_ids=gerente_ids, machine_models=MACHINE_MODELS)
...
def get_auxiliaries_by_role(user_id):
    ...
    return {
        'motoristas': [{'id': motorista['id'], 'name': motorista['name']} for motorista in motoristas],
        'gerentes': [{'id': gerente['id'], 'name': gerente['name']} for gerente in gerentes]
    }
...
def delete_machine(machine_id):
    ...
    with connect_db() as conn:
        conn.execute('DELETE FROM machines WHERE id = ? AND user_id = ?', (machine_id, user_id))
        conn.commit()
    ...
    if request.method == "POST":
        try:
            ...
            with connect_db() as conn:
                ...
                auxiliary_chat_ids = {key: value for key, value in request.form.items() if key.startswith("auxiliary_chat_id_")}
                auxiliary_roles = {key: value for key, value in request.form.items() if key.startswith("auxiliary_role_")}
...
def get_auxiliaries(user_id):
    ...
    return [{'name': aux[0], 'email': aux[1], 'celular': aux[2], 'chat_id': aux[3], 'role': aux[4]} for aux in auxiliaries]
...
def send_report():
    ...
    # Validação básica
    if not recipient:
        flash("Por favor, insira o número de telefone ou chat ID/username.")
        return redirect(url_for("dashboard"))

    message = "Relatório teste"
    conversation_service = ConversationService()

    try:
        # Enviar mensagem para o canal selecionado
        response = conversation_service.send_message(message, channel, recipient)
        flash(
            f"Relatório enviado com sucesso para {recipient} via {channel.capitalize()}!"
        )
        print(response)
    except Exception as e:
        flash(f"Erro ao enviar relatório: {e}")
